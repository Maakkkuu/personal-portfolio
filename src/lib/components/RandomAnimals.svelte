<script lang="ts">
	...
	let randomAnimals: { src: string; top: number; left: number; scale: number; rotation: number }[] =
		$state([]);
...
<div class="pointer-events-none fixed inset-0 z-0 overflow-hidden">
	{#each randomAnimals as animal}
		<img
			...
			class="absolute opacity-20 grayscale transition-all duration-500 ease-in-out hover:opacity-80 hover:grayscale-0"
			style="
                top: {animal.top}%;
                left: {animal.left}%;
